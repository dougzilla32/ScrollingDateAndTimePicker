...
open class LoadableFromXibView: UIView {
    ...
    func xibSetup() {
        view = LoadableFromXibView.xibView(owner: self)
        addSubview(view)
    ...
    static func xibView(owner: Any) -> UIView? {
        return xib(viewClass: type(of: owner) as! AnyClass)?.instantiate(withOwner: owner, options: nil).first as? UIView
    }
    
    static func xibView(viewClass: AnyClass) -> UIView? {
        return xib(viewClass: viewClass)?.instantiate(withOwner: nil, options: nil).first as? UIView
    }
    
    private static func xib(viewClass: AnyClass) -> UINib? {
        return UINib(nibName: String(describing: viewClass), bundle: Bundle.module)
    }
